...
answer to. **Settings → Window & Tabs → Sidebar git display → Allow clicks to
open diff preview** controls clicks in both places. The display mode itself only
changes the sidebar; Info keeps its branch and counts even in **Hidden** mode.
