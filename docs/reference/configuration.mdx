...
| `sidebar_git_display` | enum | `"full"` | Sidebar rows and group headers: `full` shows branch icon, branch name and counts; `counts` shows only `+N −M` (no empty line for clean repos); `hidden` hides these git details. Does not change the Info panel. |
| `sidebar_diff_preview` | bool | `true` | Independent click preference for `+N −M` in the sidebar and Info panel. Preserved when the display mode changes. |
