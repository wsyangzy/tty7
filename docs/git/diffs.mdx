...
Turn off **Settings → Window & Tabs → Sidebar git display → Allow clicks to open
diff preview** (`sidebar_diff_preview: false`) to disable count clicks in both
the sidebar and Info panel.

The separate **Sidebar git display** setting (`sidebar_git_display`) chooses
**Full** (`full`), **Only +N −N** (`counts`), or **Hidden** (`hidden`) for sidebar
rows and group headers. Switching modes preserves the click preference. These
settings do not disable opening diffs from Source Control or History.
