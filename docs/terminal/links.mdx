...
Paths wrapped by the terminal are detected across rows. A path enclosed in
parentheses, brackets, or quotes can also span explicit line breaks with
padding or indentation, as in coding-agent output. Hovering, clicking, and
the file context menu all use the complete path.
